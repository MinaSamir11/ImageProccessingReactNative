...
class Camera extends Component {
    async ProcessImageProccessing() {
        if (this.camera) {
            ...
            }else if(!this.state.FrontAnalysisImage || !this.state.LeftFrontAnalysisImage){
                console.log('awe');
      ...
      try {
        if (analyses.length > 10) {
            if (index === 1) {
              ...
              if(this.state.FrontAnalysisImage.length > 1){
                  if(this.state.LeftFrontAnalysisImage){
                      if(this.state.FrontAnalysisImage.length < this.state.LeftFrontAnalysisImage.length){
                        this.setState({
          ...
          if (this.state.FrontAnalysisImage) {
            ...
            if(index === 1){
                this.setState({
                    txt_instructions: 'Go to left front of the car to capture photo you must include with picture more details around the car or test will be quit '
                });    
            ...
            if (this.state.LeftFrontAnalysisImage) {
              ...
              for (var i = 0; i < this.state.FrontAnalysisImage.length - 1; i++) {
                ...
                if(this.state.filter.length < 8){                
                   ...
                   Snackbar.show({
                    text:
                      'Image is not the same value is this your car !',
                    duration: Snackbar.LENGTH_LONG,
                    action: {
                      text: 'UNDO',
                      textColor: 'green',
               ...
               this.setState({
                txt_instructions: 'All Done Front and centre are the Same Values',
                stateLoading: false,
                btn_command:'Done'
              });
    ...
    async DetectShasehNo(uri) {
        ...
        if(this.props.handleInputs === labels.text){
            ...
            await this.setState({
               txt_instructions: 'Go to the centre of the car To capture photos you must include with picture more details around the car or test will quit',
               btn_command: 'All Done',
               ShasehNo: labels.text
              });
           await this.ShowAlertWithDelay();
          }else{
              this.setState({
            ...
            <View style={Styles.container}>
                ...
                <View style={Styles.controls}>
                    ...
                    <View style={Styles.ActivityIndicatior}>
                        <ActivityIndicator style={Styles.Indicator} animating={this.state.stateLoading} color={'#fff'} size={23} />
                        <Text style={Styles.txt_Instructions}>{this.state.txt_instructions}</Text>
                        {this.state.Show_btn_command &&
                            <Button style={Styles.btn_command}
                                ...
                                onPress={() => {
                                    if (this.state.btn_command === "Let's Start") {
                                        this.setState({
                                            btn_command: 'Reading...',
                                            stateLoading: false,
                                            txt_instructions: 'Go to Shaseh Number and hold on while See all done'
                                        });
                                        this.ShowAlertWithDelay();
                                ...
                                }}> <Text style={Styles.txt_btn_command}>{this.state.btn_command}</Text> </Button>
                        }
                    </View>
